...
bool ifBST(BinaryTreeNode<int> *root){    //IF ROOT IS GREATERR THAN LEFT TREE'S MAXIMUM AND LESS THAN RIGHT TREE'S MINIMUM
    if(root == NULL){
        return true;
    }
    int leftMax = maximum(root->left);
    int rightMin = minimum(root->right);
    bool output = (root->data > leftMax ) && (root->data<rightMin) && ifBST(root->left) && ifBST(root->right);
    return output;
}
